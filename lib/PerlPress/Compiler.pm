...
sub list_cat
{
    ...
    my $cat_dir = $arg_ref->{'cat_dir'}
        or croak "Specify local base directory for category files!\n";
...
sub nav_menu
{
    ...
    # Check if all necessary arguments are present
    my $dbh = $arg_ref->{'dbh'} or croak "Specify database handler!\n";
    my $dirs = $arg_ref->{'dirs'} or croak "Specify directories!\n";
    my $refdir = $arg_ref->{'refdir'}
        or croak "Specify reference directory!\n";
    ...
    push @menu, {
        NAV_NAME => "Home",
        NAV_LINK => $dirs->{'base'}->file("index.html")->relative($refdir)
    };
...
sub html_art
{
    ...
    my $dirs = $arg_ref->{'dirs'} or croak "Specify directory information!\n";
    my $templ = $arg_ref->{'templ'} or croak "Specify template!\n";
    ...
    $art->{'created_epoch'} = PerlPress::Tools::date_str2epoch({
        date => $art->{created}
    });
    $art->{'modified_epoch'} = PerlPress::Tools::date_str2epoch({
        date => $art->{modified}
    });
    ...
    my @art_cat_alias = @{PerlPress::DBAcc::get_cat_alias({
        dbh    => $dbh,
        cat_id => \@art_cat_ids
    })};
    my @art_cats = ();
    foreach my $c (0 .. $#art_cat_names)
    {
        push @art_cats, {
            ART_CAT_LINK =>
                $dirs->{"cat_".$art_cat_ids[$c]}->file("index.html")->relative($refdir),
    ...
    my @art_tag_alias = @{PerlPress::DBAcc::get_tag_alias({
        dbh    => $dbh,
        tag_id => \@art_tag_ids })};
    my @art_tags = ();
    ...
    {
        push @art_tags, {
            ART_TAG_LINK => $dirs->{"tag_".$art_tag_ids[$t]}->file("index.html")->relative($refdir),
            ART_TAG_NAME => $art_tag_names[$t] };
    }
	
    # Get the code for the navigation menu
    my @nav_menu = @{PerlPress::Compiler::nav_menu({
        dbh    => $dbh,
        dirs   => $dirs,
        refdir => $refdir
    })};
    ...
    my @html_shortcuts;
    my @shorts = @{PerlPress::DBAcc::get_shorts_list({
        dbh=>$dbh
    })};
    foreach my $s (@shorts)
    {
    ...
    my @predef = @{PerlPress::Compiler::predef_shortcuts({
        dbh    => $dbh,
        dirs   => $dirs,
        refdir => $refdir
    })};
    ...
    my $html = HTML::Template->new(
        ...
        loop_context_vars => 1,
        utf8              => 1
    ) or croak "Cannot open the template file $templ, $!\n";

    $html->param(
        CHARSET       => "utf-8",
        META_AUTHOR   => $art->{'author'},
        META_DESCR    => PerlPress::Tools::striphtml({
            html      => $art->{'intr_text'},
            max_len   => 150
        }),
        META_KEYWORDS => join(", ", @art_tag_names),
        META_DATE     => PerlPress::Tools::epoch2date_str({
                             date      => $art->{'created_epoch'},
                             tz_offset => 1,
        ...
        ART_LIST      => [ {
            ...
            ART_TITLE       => $art->{'title'},
            ART_INFO_SHOW   => 1,
            ART_DATE        => PerlPress::Tools::epoch2date_str({
                                   date   => $art->{'created_epoch'},
                                   format => "DD.MM.YYYY"
                               }),
    ...
    open my $filehandle, ">", $file or croak "Could not open $file: $!";
...
sub html_list
{
    ...
    # Check if all necessary arguments are present
    my $dbh = $arg_ref->{'dbh'} or croak "Specify database handler!\n";
    my $type = $arg_ref->{'type'} or croak "Specify type (cat|tag|blog)!\n";
    my $dirs = $arg_ref->{'dirs'}
        or croak "Specify directory information!\n";
    my $templ = $arg_ref->{'templ'} or croak "Specify template!\n";
    ...
    given($type)
    {
        when(m/(cat|category)/i)
        {
            ...
            my $id = $arg_ref->{'id'} or croak "Specify category id!\n";
            $refdir = $dirs->{"cat_".$id};

            $cattag_name = ${PerlPress::DBAcc::get_cat_name({
                dbh    => $dbh,
                cat_id => [$id]
            })}[0];
            @art_ids = @{PerlPress::DBAcc::get_art_list_by_cat({
                dbh    => $dbh,
                cat_id => $id
            })};
        }
        when(m/tag/i)
        {
            ...
            my $id = $arg_ref->{'id'} or croak "Specify tag id!\n";
            $refdir = $dirs->{"tag_".$id};

            $cattag_name = ${PerlPress::DBAcc::get_tag_name({
                dbh    => $dbh,
                tag_id => [$id]
            })}[0];
            @art_ids = @{PerlPress::DBAcc::get_art_list_by_tag({
                dbh    => $dbh,
                tag_id => $id
            })};
        }
        when(m/blog/i)
        {
            ...
            @art_ids = @{PerlPress::DBAcc::get_publ_art({
                dbh=>$dbh,
                blog_only=>1
            })};
        }
        default
        {
            croak "Invalid type argument (not \"cat\", \"tag\" or \"blog\")";
        }
    }
    ...
    while($#art_ids >= 0)
    {
        ...
        for my $art_id (@art_ids_page)
        {
            ...
            # Calculate some additional data
            $art->{'created_epoch'} =
                PerlPress::Tools::date_str2epoch({ date=>$art->{created} });
            $art->{'modified_epoch'} =
                PerlPress::Tools::date_str2epoch({ date=>$art->{modified} });
            ...
            {
                push @art_cats, {
                    ART_CAT_LINK => $dirs->{"cat_".$art_cat_ids[$c]}->file("index.html")->relative($refdir),
                    ART_CAT_NAME => $art_cat_names[$c] };
            }
...
sub sitemap
{
    ...
    open my $fh,">:utf8", $dirs->{'sitemap'}
        or croak "Cannot open sitemap file $!";
    ...
    foreach my $art_id (@publ)
    {
        ...
        my $lastmod = PerlPress::Tools::epoch2date_str({
            date => $art->{'created_epoch'},
            format=>"YYYY-MM-DD"
        });
...
sub art_link
{
    ...
    my $dbh = $arg_ref->{'dbh'} or croak "Specify database handler!\n";
    my $dirs=$arg_ref->{'dirs'} or croak "Specify directory information!\n";
    my $refdir=$arg_ref->{'refdir'} or croak "Specify reference directory!\n";
    my $art_id=$arg_ref->{'art_id'} or croak "Specify article id!\n";
...
sub predef_shortcuts
{
    ...
    push @predef, {
        find => "\{article id=\"(?<id>.*)\"\}(?<txt>[^\{]*)\{\/article\}",
        repl => sub{PerlPress::Compiler::art_link({
                        dbh=>$dbh,
                        dirs=>$dirs,
                        refdir=>$refdir,
                        art_id=>$+{id},
