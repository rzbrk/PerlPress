...
sub html_art
{
	...
	$html->param(CHARSET => "utf-8",
				 ...
				 META_DATE => PerlPress::Tools::epoch2date_str({ date=>$art->{'created_epoch'}, tz_offset=>1, format=>"YYYY-MM-DDThh:mm:ssTZD" }),
...
sub html_list
{
	...
	while($#art_ids >= 0)
	{
		...
		$html->param(CHARSET => "utf-8",
					 ...
					 META_DESCR => "",
					 META_KEYWORDS => "",
					 META_DATE => PerlPress::Tools::epoch2date_str({ date=>time, tz_offset=>1, format=>"YYYY-MM-DDThh:mm:ssTZD" }),
